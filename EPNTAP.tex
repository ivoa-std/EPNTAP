...
physical parameters), access, references, etc. The notion of ``parameter" is 
...
with the ObsCore standard, from which it borrows several concepts.

EPN-TAP also uses the notion of ``granule'' (inherited from the SPASE standard)
...
according to TAP guidelines (sect.~\ref{sect:registry}).
...
\item Values of string parameters support internal spaces
(not leading or trailing spaces).

\item Special characters and quotes are not allowed in string parameter values,
...
below. The data provider must select the type most adapted to their
...
\begin{verbatim}
SELECT * FROM spectro_asteroids.epn_core
...
\begin{verbatim}
SELECT * FROM spectro_asteroids.epn_core WHERE time_min > '2455197.5' AND time_max < '2455927.5'
\end{verbatim}
...
\begin{verbatim}
SELECT * FROM spectro_asteroids.epn_core WHERE time_min BETWEEN '2455197.5' and '2455927.5'
\end{verbatim}
...
coordinates are not defined (e.g. a non-zero value is required by the mixin
in DaCHS).
...
Services which need to identify data
...
\textbf{\\}
\textbf{Use of general parameters}
...
during the Europlanet-RI programme, and was limited to test purposes.
