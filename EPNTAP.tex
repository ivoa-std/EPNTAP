...
As a rule of thumb, instrument\_name implies one or more coverages 
...
via the spectral\_range\_min/max parameters --- this
...
time\_min&Double&d&Start time (in JD). UTC measured at time\_refposition location (default is observer's frame)&\ucd{time.start;obs}\\
time\_max&Double&d&Stop time (in JD). UTC measured at time\_refposition location (default is observer's frame)&\ucd{time.end;obs}\\
...
bib\_reference&Text&&Bibcode or doi recommended; can be a URL or anything else. Refers to the \emph{granule}&\ucd{meta.bib}\\
