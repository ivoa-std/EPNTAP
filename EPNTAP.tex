...
(sections~2 and 3) and service registration
...
These 3 parameters must always be provided. If granule grouping is not
...
(i.e., samples), but the target\_class parameter must be provided in
...
\begin{itemize}
\item Search data contained in a time range:
\end{itemize}
...
spectral_range_min  = c*1E6 / spectral_range_max_micron
spectral_range_max  = c*1E6 / spectral_range_min_micron
...
after a query. These 4 parameters must always be provided.
...
select distinct(schema_name), table_utype from rr.res_schema natural join rr.res_table where table_utype LIKE '%epn%'
\end{verbatim}
...
This section describes optional parameters of general use, while section 2.3 describes 
...
This parameter refers to the publisher of the \emph{data service}, not necessarily at
the origin of the data. Provided as a free format string.
...
target in the FoV). If this parameter is not provided, time is expected
...
They can be used independently, like the generic optional parameters described in section 2.2. 
...
Give broad location and geographic position of the observer or telescope. observer\_location can be used when the exact location cannot be released  (e.g., small telescopes).\\
...
VOEvents \citep{2006ivoa.spec.1101S} can be archived in EPN-TAP tables. The following extra parameters are available to complete their metadata:
