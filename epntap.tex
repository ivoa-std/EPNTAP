...
\ivoagroup{Data Models, Data Access Layer}
...
s\_region&spoly&Note~\ref{atn-3}&ObsCore-like footprint in 2D (celestial or body-related frames)&\ucd{phys.outline;obs.field}\\
...
\textbf{creation\_date}&Timestamp&&Date of first entry of this granule&\ucd{time.creation}\\
\textbf{modification\_date}&Timestamp&&Date of last modification&\ucd{time.processing}\\
\textbf{release\_date}&Timestamp&&Start of public access period (set to creation\_date if no proprietary period)&\ucd{time.release}\\
...
datalink\_url&Text&&Provides links to files or services on the server&\ucd{meta.ref.url;meta.datalink}\\
species&Text&&Identifies a chemical species, \emph{case sensitive}&\ucd{meta.id;phys.atmol}\\
waveband&Text&&Electro-magnetic band, from enumerated list&\ucd{instr.bandpass}\\
alt\_target\_name&Text&&Provides alternative target name(s). Can be a hash list&\ucd{meta.id;src}\\
target\_region&Text&&Type of region or feature of interest&\ucd{obs.field}\\
feature\_name&Text&&Secondary name (e.g. standard name of a region of interest)&\ucd{obs.field}\\
...
time\_origin&Text&&Defines \emph{where} the time is measured (e. g., ground vs spacecraft). Default is observer's frame.&\ucd{meta.ref;time.scale}\\
...
local\_time\_min&Double&h&Min local time at observed region&\ucd{time.period.rotation;time.phase;stat.min}\\
local\_time\_max&Double&h&Max local time at observed region&\ucd{time.period.rotation;time.phase;stat.max}\\
...
target\_time\_min&Timestamp&&Min observing time in target frame&\ucd{time.start;src}\\
target\_time\_max&Timestamp&&Max observing time in target frame&\ucd{time.end;src}\\
...
proposal\_id&Integer&&Proposal identifier&\ucd{meta.id;obs.proposal}\\
...
campaign&Text&&Name of the observational campaign&\ucd{meta.id;meta.code}\\
...
target\_apparent\_radius&Double&&Apparent radius of the target&\ucd{phys.angSize;src}\\
...
orientation&Double&&Position angle of image y axis (on sky only)&\ucd{pos.posAng}\\
observer\_name&Text&&Observer name&\ucd{obs.observer;meta.main}\\
observer\_institute&Text&&Observer institute&\ucd{meta.note;meta.main}\\
observer\_id&Integer&&Image observer's PVOL numeric identifier&\ucd{meta.id.PI}\\
...
observer\_lon&Double&&Observer's approximate longitude&\ucd{obs.observer;pos.earth.lon}\\
observer\_lat&Double&&Observer's approximate latitude&\ucd{obs.observer;pos.earth.lat}\\
mass&Double&kg&Mass of object&\ucd{phys.mass}\\
sideral\_rotation\_period&Double&h&Object rotation rate&\ucd{time.period.rotation}\\
...
original\_publisher&Text&&Refers to the source of the data, e. g.,  in compilations of experimental data&\ucd{meta.note;meta.main}\\
producer\_name&Text&&Data producer name, especially in compilations of experimental data&\ucd{meta.note;meta.main}\\
producer\_institute&Text&&Data producer institute, e. g., in compilations of experimental data&\ucd{meta.note;meta.main}\\
