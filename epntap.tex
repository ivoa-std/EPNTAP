...
\def\ucd#1{{\sl #1}}
\else
\def\ucd{\startucd\realucd}
\def\realucd#1{\sl#1\enducd}
\begingroup
% let LaTeX break UCD componds and at dots and semicolons in UCDs
\gdef\breakabledot{.\hskip0pt}
\gdef\breakablesemicolon{;\hskip0pt}
\catcode`\.=\active\catcode`\;=\active
\gdef\startucd{\begingroup
  \catcode`\.=\active\let.=\breakabledot
  \catcode`\;=\active\let;=\breakablesemicolon}
...
\item Parameters accept values in lower case, except when a global standard or vocabulary applies. This is the case in particular with Solar System objet names, which follow the IAU naming convention. 

\item Empty parameters can be void or can contain the NULL value. 

\item Some floating-point parameters need to be stored and returned in double precision to preserve required accuracy. The base line is to store all such parameters in double precision.
...
\item \textbf{pf }(= photometric function): scalar or vectorial measurements along 1 angular dimension, e. g., phase or polarization curves, phase functions, emission-phase function sequences… Does not handle variations along several angular axes.
...
\sptablerule
\vbox{\vskip 2pt\hbox{EPN-}\vskip 3pt\hbox{TAP2}}&CODMAC&PSA&NASA&PDS3&PDS4&ObsTAP&Description\\
\sptablerule
...
2?&\_&&&\_&Partially calibrated&&Processed beyond the raw stage, but have not yet reached calibrated status (PDS4)\\
...
\item It is however good practice to associate dwarf planet\#asteroid to remove any ambiguity, e. g. to support older catalogues.
...
Each quantity is a set of 2 parameters providing min and max values (except spatial\_frame\_type, s\_region, and shape). They must both be present with the same value when min = max.
...
1) The original ObsCore s\_region one (currently limited to polygons) — a string with syntax: '\{(lon1,lat1), (lon2,lat2), … \}' (with no quotes) where (lon1,lat1) = (10.d, 5d), character d included (it stands for degrees). Pairs (lon1,lat1) must sample the contour in a sequence provided in direct (counter-clockwise) sense as viewed from the observer. In an output VOTable, this could be represented as an STC-S string, which has different format (as in ObsCore). The FIELD elements of such serialisations are marked up with an xtype = ``adql:REGION'' attribute.

2) Modern DALI 1.1 geometries —  these can provide either points ('lon lat', with no quotes), circles ('lon lat radius', with no quotes), or polygons (sequence of at least 3 'lon lat' pairs, with no quotes). For polygons, pairs (lon lat) must sample the contour in a sequence provided in direct (counter-clockwise) sense as viewed from the origin (when on the sky) or from the observer (when on planets). In the VOTable serialisation, the corresponding FIELD elements are marked up with an xtype='point', 'circle', or 'polygon' attribute.
...
This parameter is a generalization of waveband from VODataService (used e. g. in ConeSearch and SSA). It provides a rough indication of the spectral domain, e. g. when variable in a large archive. In addition, values for messengers can describe particles other than photons. Possible values are from the IVOA vocabulary (and case matters) https://www.ivoa.net/rdf/messenger/2020-08-26/messenger.html\footnote{\url{https://www.ivoa.net/rdf/messenger/2020-08-26/messenger.html}}.
...
time\_sampling\_step\_min&Double&s&Min time sampling step&\ucd{time.resolution;stat.min}\\
time\_sampling\_step\_max&Double&s&Max time sampling step&\ucd{time.resolution;stat.max}\\
time\_exp\_min&Double&s&Min integration time&\ucd{time.duration;obs.exposure;stat.min}\\
time\_exp\_max&Double&s&Max integration time&\ucd{time.duration;obs.exposure;stat.max}\\
spectral\_range\_min&Double&Hz&Min spectral range (as frequency)&\ucd{em.freq;stat.min}\\
spectral\_range\_max&Double&Hz&Max spectral range (as frequency)&\ucd{em.freq;stat.max}\\
spectral\_sampling\_step\_min&Double&Hz&Min spectral sampling step&\ucd{em.freq;stat.interval;stat.min}\\
spectral\_sampling\_step\_max&Double&Hz&Max spectral sampling step&\ucd{em.freq;stat.interval;stat.max}\\
spectral\_resolution\_min&Double&&Min spectral resolution (resolving power)&\ucd{spect.resolution;stat.min}\\
spectral\_resolution\_max&Double&&Max spectral resolution (resolving power)&\ucd{spect.resolution;stat.max}\\
c1min&Double&Note~\ref{atn-1}&Min of first coordinate, depends on the frame.&see table below\\
c1max&Double&Note~\ref{atn-1}&Max of first coordinate, depends on the frame&\\
c2min&Double&Note~\ref{atn-1}&Min of second coordinate, depends on the frame.&\\
c2max&Double&Note~\ref{atn-1}&Max of second coordinate, depends on the frame&\\
c3min&Double&Note~\ref{atn-1}&Min of third coordinate&\\
c3max&Double&Note~\ref{atn-1}&Max of third coordinate&\\
s\_region&spoly&Note~\ref{atn-3}&ObsCore-like footprint in 2D (celestial or body-related frames)&\ucd{pos.outline;obs.field}\\
c1\_resol\_min&Double&Note~\ref{atn-2}&Min resolution in first coordinate&Note~\ref{atn-2}\\
c1\_resol\_max&Double&Note~\ref{atn-2}&Max resolution in first coordinate&Note~\ref{atn-2}\\
c2\_resol\_min&Double&Note~\ref{atn-2}&Min resolution in second coordinate&Note~\ref{atn-2}\\
c2\_resol\_max&Double&Note~\ref{atn-2}&Max resolution in second coordinate&Note~\ref{atn-2}\\
c3\_resol\_min&Double&Note~\ref{atn-2}&Min resolution in third coordinate&Note~\ref{atn-2}\\
c3\_resol\_max&Double&Note~\ref{atn-2}&Max resolution in third coordinate&Note~\ref{atn-2}\\
...
incidence\_min&Double&deg&Min incidence angle (solar zenithal angle)&\ucd{pos.incidenceAng;stat.min}\\
incidence\_max&Double&deg&Max incidence angle (solar zenithal angle)&\ucd{pos.incidenceAng;stat.max}\\
emergence\_min&Double&deg&Min emergence angle&\ucd{pos.emergenceAng;stat.min}\\
emergence\_max&Double&deg&Max emergence angle&\ucd{pos.emergenceAng;stat.max}\\
phase\_min&Double&deg&Min phase angle&\ucd{pos.phaseAng;stat.min}\\
phase\_max&Double&deg&Max phase angle&\ucd{pos.phaseAng;stat.max}\\
...
messengers&Text&&Electro-magnetic band, from enumerated list&\ucd{instr.bandpass}\\
...
subsolar\_longitude\_min&Double&deg&Minimum sub-solar point longitude&\ucd{pos.bodyrc.lon;stat.min}\\
subsolar\_longitude\_max&Double&deg&Maximum sub-solar point longitude&\ucd{pos.bodyrc.lon;stat.max}\\
subsolar\_latitude\_min&Double&deg&Minimum sub-solar point latitude&\ucd{pos.bodyrc.lat;stat.min}\\
subsolar\_latitude\_max&Double&deg&Maximum sub-solar point latitude&\ucd{pos.bodyrc.lat;stat.max}\\
subobserver\_longitude\_min&Double&deg&Minimum sub-observer point longitude (sub-Earth for ground based observations)&\ucd{pos.bodyrc.lon;stat.min}\\
subobserver\_longitude\_max&Double&deg&Maximum sub-observer point longitude (sub-Earth for ground based observations)&\ucd{pos.bodyrc.lon;stat.max}\\
subobserver\_latitude\_min&Double&deg&Minimum sub-observer point latitude (sub-Earth for ground based observations)&\ucd{pos.bodyrc.lat;stat.min}\\
subobserver\_latitude\_max&Double&deg&Maximum sub-observer point latitude (sub-Earth for ground based observations)&\ucd{pos.bodyrc.lat;stat.max}\\
ra&Double&deg&Right ascension&\ucd{pos.eq.ra;meta.main}\\
dec&Double&deg&Declination&\ucd{pos.eq.dec;meta.main}\\
radial\_distance\_min&Double&km&Min distance from observed area to body center&\ucd{pos.distance;pos.bodyrc;stat.min}\\
radial\_distance\_max&Double&km&Max distance from observed area to body center&\ucd{pos.distance;pos.bodyrc;stat.max}\\
altitude\_fromshape\_min&Double&km&Min altitude of observed area above shape model / DTM&\ucd{pos.bodyrc.alt;stat.min}\\
altitude\_fromshape\_max&Double&km&Max altitude of observed area above shape model / DTM&\ucd{pos.bodyrc.alt;stat.max}\\
solar\_longitude\_min&Double&deg&Min Solar longitude Ls (location on orbit / season)&\ucd{pos.ecliptic.lon;pos.heliocentric;stat.min}\\
solar\_longitude\_max&Double&deg&Max Solar longitude Ls (location on orbit / season)&\ucd{pos.ecliptic.lon;pos.heliocentric;stat.max}\\
local\_time\_min&Double&h&Min local time at observed region&\ucd{time.phase;time.period.rotation;stat.min}\\
local\_time\_max&Double&h&Max local time at observed region&\ucd{time.phase;time.period.rotation;stat.max}\\
target\_distance\_min&Double&km&Min observer-target distance&\ucd{pos.distance;stat.min}\\
target\_distance\_max&Double&km&Max observer-target distance&\ucd{pos.distance;stat.max}\\
...
earth\_distance\_min&Double&AU&Min Earth-target distance&\ucd{pos.distance;stat.min}\\
earth\_distance\_max&Double&AU&Max Earth-target distance&\ucd{pos.distance;stat.max}\\
sun\_distance\_min&Double&AU&Min Sun-target distance&\ucd{pos.distance;stat.min}\\
sun\_distance\_max&Double&AU&Max Sun-target distance&\ucd{pos.distance;stat.max}\\
...
target\_apparent\_radius&Double&arcsec&Apparent radius of the target&\ucd{phys.angSize;src}\\
north\_pole\_position&Double&deg&North pole position angle with respect to celestial north pole&\ucd{pos.posAng}\\
...
platesc&Double&arcsec/pix&pixel angular size or platescale (on sky only)&\ucd{instr.scale}\\
orientation&Double&deg&Position angle of image y axis (on sky only)&\ucd{pos.posAng}\\
...
observer\_lon&Double&deg&Observer's approximate longitude&\ucd{obs.observer;pos.earth.lon}\\
observer\_lat&Double&deg&Observer's approximate latitude&\ucd{obs.observer;pos.earth.lat}\\
mass&Double&kg&Mass of object&\ucd{phys.mass}\\
sidereal\_rotation\_period&Double&h&Object rotation rate&\ucd{time.period.rotation}\\
mean\_radius&Double&km&&\ucd{phys.size.radius}\\
equatorial\_radius&Double&km&&\ucd{phys.size.radius}\\
polar\_radius&Double&km&&\ucd{phys.size.radius}\\
diameter&Double&km&Target diameter, or equivalent diameter for binary objects&\ucd{phys.size.diameter}\\
semi\_major\_axis&Double&AU&&\ucd{phys.size.smajAxis}\\
inclination&Double&&Orbit inclination&\ucd{src.orbital.inclination}\\
eccentricity&Double&&Orbit eccentricity&\ucd{src.orbital.eccentricity}\\
long\_asc&Double&deg&Longitude of ascending node, J2000.0&\ucd{src.orbital.node}\\
arg\_perihel&Double&deg&Argument of Perihelion, J2000.0&\ucd{src.orbital.periastron}\\
mean\_anomaly&Double&deg&Mean anomaly at the epoch&\ucd{src.orbital.meanAnomaly}\\
...
magnitude&Double&mag&Absolute magnitude. For small bodies, from HG magnitude system&\ucd{phys.magAbs}\\
flux&Double&mJy&Target flux&\ucd{phot.flux.density}\\
albedo&Double&&Target albedo&\ucd{phys.albedo}\\
map\_projection&Text&&ID from enumerated list, or string with parameters (referring to a standard)&\emph{pos.projection}\\
map\_height&Double&pix&Map size in px&\ucd{phys.size}\\
map\_width&Double&pix&Map size in px&\ucd{phys.size}\\
map\_scale&Text&&Format TBD&\ucd{pos.wcs.scale}\\
pixelscale\_min&Double&km/pix&Min pixel size on a surface&\ucd{instr.scale;stat.min}\\
pixelscale\_max&Double&km/pix&Max pixel size on a surface&\ucd{instr.scale;stat.max}\\
particle\_spectral\_type&Text&&From enumerated list&\ucd{meta.id;phys.particle}\\
particle\_spectral\_range\_min&Double&&&\ucd{phys.energy;phys.particle;stat.minphys.mass;phys.particle;stat.min}\\
particle\_spectral\_range\_max&Double&&&\ucd{phys.energy;phys.particle;stat.maxphys.mass;phys.particle;stat.max}\\
particle\_spectral\_sampling\_step\_min&Double&&&\ucd{spect.resolution;phys.particle;stat.min}\\
particle\_spectral\_sampling\_step\_max&Double&&&\ucd{spect.resolution;phys.particle;stat.max}\\
particle\_spectral\_resolution\_min&Double&&&\ucd{spect.resolution;phys.particle;stat.min}\\
particle\_spectral\_resolution\_max&Double&&&\ucd{spect.resolution;phys.particle;stat.max}\\
...
grain\_size\_min&Double&um&Min sample particle size&\ucd{phys.size;stat.min}\\
grain\_size\_max&Double&um&Max sample particle size&\ucd{phys.size;stat.max}\\
azimuth\_min&Double&deg&Min azimuth angle for illumination&\ucd{pos.azimuth;stat.min}\\
azimuth\_max&Double&deg&Max azimuth angle for illumination&\ucd{pos.azimuth;stat.max}\\
pressure&Double&bar&Ambient pressure&\ucd{phys.pressure}\\
...
temperature&Double&K&Ambient temperature&\ucd{phys.temperature}\\
...
\item\label{atn-2} Spatial resolution parameters same unit as spatial
...
\item\label{atn-3} Any type that works with ADQL's geometry operators
(CONTAINS, INTERSECTS...) is legal here.

\item\label{atn-4} Timestamps are provided as ISO-8601 strings.
\end{enumerate}

\begin{sidewaystable}
\begin{tabular}{p{0.1\textwidth}p{0.1\textwidth}p{0.2\textwidth}p{0.13\textwidth}p{0.13\textwidth}p{0.13\textwidth}p{0.1\textwidth}}
\sptablerule
&\bf celestial&\bf body&\bf cartesian
  &\bf spherical&\bf cylindrical&\bf healpix\\
\sptablerule
\bf c1min
  &\ucd{pos.eq.ra;stat.min}
  &\ucd{pos.bodyrc.lon;stat.min}
  &\ucd{pos.cartesian.x;stat.min} (in km)
  &\ucd{pos.distance;stat.min} (beuh) or \ucd{pos.radius;stat.min} 
    (does not exist)
  &\ucd{pos.distance;stat.min} (beuh) or \ucd{pos.radius;stat.min} 
    (does not exist)
  & IVOA? \\
...
\bf c2min	
  ...
  &\ucd{pos.angDistance;stat.min}
    or \ucd{pos.az.zd;stat.min}?	
  &\ucd{pos.az.azi;stat.min}	\\

\bf c2max 
  &\ucd{pos.eq.dec;stat.max}
  &\ucd{pos.bodyrc.lat;stat.max}
  &\ucd{pos.cartesian.y;stat.max} (in km)
  &\ucd{pos.angDistance;stat.max} or \ucd{pos.az.zd;stat.max} ?
  &\ucd{pos.az.azi;stat.max}\\
...
\bf c3min	
  ...
  &\ucd{pos.AngDistance;stat.min} or \ucd{pos.az.azi;stat.min}?
  &\ucd{pos.distance;stat.min} (height) (in km) \\
...
\bf c3max	
  ...
  &\ucd{pos.AngDistance;stat.max} or \ucd{pos.az.azi;stat.max}?
  &\ucd{pos.distance;stat.max} (height) (in km)\\
\sptablerule
\end{tabular}
